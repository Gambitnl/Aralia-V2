  ...
  for (let r = 0; r < rows; r++) {
    ...
    for (let c = 0; c < cols; c++) {
      tiles[r][c] = {
        x: c,
        y: r,
        biomeId: passableBiomeIds[Math.floor(Math.random() * passableBiomeIds.length)], // Initial random assignment
        discovered: false,
  ...
  for (let i = 0; i < 3; i++) { // Multiple passes for slightly better clustering
    for (let r = 0; r < rows; r++) {
      for (let c = 0; c < cols; c++) {
        ...
        if (dominantNeighborBiome && biomes[dominantNeighborBiome]?.passable && Math.random() < 0.5) {
          tiles[r][c].biomeId = dominantNeighborBiome;
        } else if (!biomes[tiles[r][c].biomeId]?.passable) { // Ensure non-location tiles are passable
           tiles[r][c].biomeId = passableBiomeIds[Math.floor(Math.random() * passableBiomeIds.length)];
        }
      }
    }
  }
