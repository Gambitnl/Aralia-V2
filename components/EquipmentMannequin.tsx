...
import { getCharacterMaxArmorProficiency, getArmorCategoryHierarchy } from '../utils/characterUtils';
...
const EquipmentMannequin: React.FC<EquipmentMannequinProps> = ({ character, onSlotClick }) => {
  ...
  const charMaxProficiencyValue = getArmorCategoryHierarchy(characterMaxProficiencyLevel === 'unarmored' ? undefined : characterMaxProficiencyLevel as ArmorCategory);
    ...
    <div className="bg-gray-700 p-4 rounded-lg shadow-inner border border-gray-600">
      ...
      >
        {equipmentSlots.map(slotInfo => {
          ...
          if (equippedItem) {
            if ((slotInfo.isArmorSlot || slotInfo.isShieldSlot) && equippedItem.armorCategory) {
              ...
              } else if (slotInfo.isArmorSlot) {
                const itemProficiencyValue = getArmorCategoryHierarchy(equippedItem.armorCategory as ArmorCategory);
          ...
          const buttonTitle = equippedItem 
            ? `${equippedItem.name} (In ${slotInfo.label})${proficiencyMismatch ? ` - Proficiency Mismatch! ${mismatchReason}` : ''}${onSlotClick ? ' - Click to Unequip' : ''}` 
          ...
          return (
            <Tooltip key={slotInfo.id} content={buttonTitle}>
                ...
                <div className={`flex flex-col items-center justify-center h-full w-full`}>
                    ...
                    {equippedItem && <span className="text-[10px] text-gray-500 mt-0.5">({slotInfo.label})</span>}
                </div>
                </button>
            </Tooltip>
          );
