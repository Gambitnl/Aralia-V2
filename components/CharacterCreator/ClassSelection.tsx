 ...
 */
const ClassSelection: React.FC<ClassSelectionProps> = ({ classes, onClassSelect, onBack }) => {
  return (
    <div>
      <h2 className="text-2xl text-sky-300 mb-6 text-center">Choose Your Class</h2>
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4 mb-6">
        {classes.map((charClass) => (
          ...
          >
            <h3 className="text-xl font-semibold text-amber-400 mb-2">{charClass.name}</h3>
            <p className="text-sm text-gray-400 mb-1">Hit Die: d{charClass.hitDie}</p>
            <p className="text-sm text-gray-400 mb-2">Primary Abilities: {charClass.primaryAbility.join(', ')}</p>
            <p className="text-xs text-gray-500" style={{ minHeight: '3em' }}>{charClass.description}</p> {/* Ensure consistent height */}
          </button>
        ))}
      </div>
      <button
