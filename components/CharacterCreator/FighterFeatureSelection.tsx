 ...
 */
const FighterFeatureSelection: React.FC<FighterFeatureSelectionProps> = ({ styles, onStyleSelect, onBack }) => {
  ...
  const handleSelect = (styleId: string) => {
    ...
    console.log(`FighterFeatureSelection: Style selected - ${styleId}`);
  };
  ...
  const handleSubmit = () => {
    if (selectedStyleId) {
      ...
      if (style) {
        console.log(`FighterFeatureSelection: Confirming style - ${style.name}`);
        onStyleSelect(style);
      }
    } else {
      console.log("FighterFeatureSelection: handleSubmit called but no style selected.");
      // Optionally, provide UI feedback if submission is attempted without selection
    }
  };
  
  // For debugging button state
  console.log(`FighterFeatureSelection: Render. selectedStyleId: ${selectedStyleId}, Button disabled: ${!selectedStyleId}`);

  return (
    <div>
      <h2 className="text-2xl text-sky-300 mb-4 text-center">Choose Fighting Style</h2>
